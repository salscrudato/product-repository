...
export interface WebVitalsConfig {
  lcp: WebVitalThreshold;
  fid: WebVitalThreshold;
  cls: WebVitalThreshold;
  fcp: WebVitalThreshold;
  ttfb: WebVitalThreshold;
}
...
export const PERFORMANCE_CONFIG: PerformanceConfiguration = {
...
export interface PerformanceBudget {
  ...
  totalImages: number;
  maxRequests: number;
  maxDomainRequests: number;
}

/**
 * Get performance budget for specific metrics
 */
export const getPerformanceBudget = (): PerformanceBudget => ({
 ...
 */
export const isMetricGood = (metricName: string, value: number): boolean => {
  const config = PERFORMANCE_CONFIG.webVitals[metricName.toLowerCase() as keyof WebVitalsConfig];
 ...
 */
export const isMetricPoor = (metricName: string, value: number): boolean => {
  const config = PERFORMANCE_CONFIG.webVitals[metricName.toLowerCase() as keyof WebVitalsConfig];
  if (!config) return false;
  return value > config.needsImprovement;
};

type DataType = 'products' | 'coverages' | 'news';

/**
 * Get cache TTL for a specific data type
 */
export const getCacheTTL = (dataType: DataType): number => {
  const ttlMap: Record<DataType, number> = {
